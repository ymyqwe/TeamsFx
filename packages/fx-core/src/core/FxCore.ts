...
export class FxCore implements v3.ICore {
  ...
  async createEnvWithNames(
    targetEnvNames: string[],
    projectSettings: ProjectSettings,
    inputs: Inputs
  ): Promise<Result<Void, FxError>> {
    for (const targetEnvName of targetEnvNames) {
      const envConfigPath = environmentManager.getEnvConfigPath(targetEnvName, inputs.projectPath!);
      if (await fs.pathExists(envConfigPath)) {
        return err(new FileExistError(envConfigPath));
      }
    }

    for (const targetEnvName of targetEnvNames) {
      const result = await this.createEnvWithName(targetEnvName, projectSettings, inputs);
      if (result.isErr()) {
        return result;
      }
    }
  ...
  async _init(inputs: Inputs, ctx?: CoreHookContext): Promise<Result<string, FxError>> {
    ...
    if (await fs.pathExists(targetReadmePath)) {
      return err(new FileExistError(targetReadmePath));
    }

    const createEnvResult = await this.createEnvWithNames(
      [environmentManager.getDefaultEnvName(), environmentManager.getLocalEnvName()],
      projectSettings,
      inputs
    );
    if (createEnvResult.isErr()) {
